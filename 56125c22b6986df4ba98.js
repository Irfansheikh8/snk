...
var grid_1 = __webpack_require__(/*! ./grid */ "../compute/grid.ts");
var getAvailableRoutes_1 = __webpack_require__(/*! ./getAvailableRoutes */ "../compute/getAvailableRoutes.ts");
var isGridEmpty = function (grid) { return grid.data.every(function (x) { return x === null; }); };
var createComputeHeuristic = function (grid0, _snake0, colors) {
    var colorCount = {};
    for (var x = grid0.width; x--;)
        for (var y = grid0.height; y--;) {
            var c = grid_1.getColor(grid0, x, y);
            if (c !== null)
                colorCount[c] = 1 + (colorCount[c] || 0);
        }
    var values = colors
        .map(function (k) { return Array.from({ length: colorCount[k] }, function () { return k; }); })
        .flat();
    return function (_grid, _snake, stack) {
        var score = 0;
        for (var i = 0; i < stack.length; i++) {
            if (stack[i] === values[i]) {
                ...
                var u = Math.abs(stack[i] - values[i]);
                score += 5 - u;
            }
        }
        return score;
...
var computeKey = function (grid, snake, stack) {
    return grid.data.map(function (x) { return x || 0; }).join("") +
        "|" +
        snake.map(function (p) { return p.x + "." + p.y; }).join(",") +
        "|" +
        stack.join("");
};
exports.getBestRoute = function (grid0, snake0, options, maxIterations) {
    if (maxIterations === void 0) { maxIterations = 500; }
    var computeHeuristic = createComputeHeuristic(grid0, snake0, options.colors);
    var closeList = {};
    var openList = [];
    {
        var h = computeHeuristic(grid0, snake0, []);
        var w = 0;
        var f = h + w;
        openList.push({
    ...
    while (openList.length && maxIterations-- > 0) {
        ...
        if (!isGridEmpty(c.grid)) {
            var availableRoutes = getAvailableRoutes_1.getAvailableRoutes(c.grid, c.snake, options, 30, 1, 20, 500);
            var _loop_1 = function (route) {
                var stack = c.stack.slice();
                var grid = grid_1.copyGrid(c.grid);
                var snake = route.snakeN;
                var _a = route.snakeN[0], x = _a.x, y = _a.y;
                stack.push(grid_1.getColor(grid, x, y));
                grid_1.setColor(grid, x, y, null);
                var key = computeKey(grid, snake, stack);
                if (!closeList[key] && !openList.some(function (s) { return s.key === key; })) {
                    var h = computeHeuristic(grid, snake, stack);
                    var w = c.w + route.directions.length;
                    var f = w - h;
                    openList.push({
                        key: key,
                        grid: grid,
            ...
            for (var _i = 0, availableRoutes_1 = availableRoutes; _i < availableRoutes_1.length; _i++) {
                var route = availableRoutes_1[_i];
                _loop_1(route);
            }
        }
    }
...
var unwrap = function (o) {
    if (!o)
        return [];
    return __spreadArrays(unwrap(o.parent), o.directions);
};
...
var getBestRoute_1 = __webpack_require__(/*! @snk/compute/getBestRoute */ "../compute/getBestRoute.ts");
var samples_1 = __webpack_require__(/*! ./samples */ "./samples.ts");
//
// init
var label = "realistic";
var _a = samples_1.samples.find(function (s) { return s.label === label; }), grid0 = _a.grid, snake0 = _a.snake, gameOptions = _a.gameOptions;
//
// compute
var s0 = performance.now();
var bestRoute = getBestRoute_1.getBestRoute(grid0, snake0, gameOptions, 120);
console.log(performance.now() - s0);
//
// draw
var draw = canvas_1.createCanvas(grid0).draw;
//
// controls
...
inputK.addEventListener("input", function () {
    ...
    var stack = [];
    for (var i = 0; i < +inputK.value; i++)
        step_1.step(grid, snake, stack, bestRoute[i], gameOptions);
